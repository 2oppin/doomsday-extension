...
export class TaskItem extends Component<ITaskItemProps, {}> {
        ...
        return (
            <>
                <_bt u="⏸" {...{readonly}} title="Suspend ..." cb={() => this.pauseTask(task)}
                     dataHelp={HelpInfo.TaskItemPause}/>
        ...
        return (
            <>
                <_bt u="▶" {...{readonly}} title="Start Working... NOW!" cb={() => this.startTask(task)}
                     dataHelp={HelpInfo.TaskItemStart}/>
    ...
    public renderFinished(task: Task) {
        ...
        return (
            <>
                <_bt u={`\ud83c\udf81`} {...{readonly}} title="Add Task to Archive" yellow={true}
    ...
    public render() {
        ...
        if (task.complete)
            return (<div className="task-item item"
                         data-help={HelpInfo.TaskItemComplete}>{this.renderFinished(task)}</div>);

        return (
            <div className="task-item item">
                {!task.priority ? null : <div className={"priority-container"} data-help={HelpInfo.TaskItemPriority}>
