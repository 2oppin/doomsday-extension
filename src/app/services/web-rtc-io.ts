...
class WebRtcIo {
  ...
  get state(): [RTCIceConnectionState, RTCSignalingState,  RTCDataChannelState] {
    return [
...
console.log("Ice Candidate:", event.candidate);
  ...
  async createOffer(): Promise<string> {
    ...
    const offerFromIce = new Promise(resolve => this.peerConnection.onicecandidate = (e) => {
...
console.log("answer-ice-answr", await answerFromIce);
  ...
  async acceptOffer(offerStr: string): Promise<string> {
    const offer: RTCSessionDescriptionInit = JSON.parse(atob(offerStr)) as undefined;
  ...
  async acceptAnswer(answerStr: string) {
    const answer: RTCSessionDescriptionInit = JSON.parse(atob(answerStr)) as undefined;
